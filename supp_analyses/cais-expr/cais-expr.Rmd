...
cais4lm <- lm(log(CAIS) ~ log(expr), data=subset(mousedf4, log(expr) > 4))
cai4lm <- lm(log(CAI) ~ log(expr), data=subset(mousedf4, log(expr) > 4))



pcais <- ggplot(mousedf4, aes(x=log(expr), y=log(CAIS))) + 
  labs( x = "log(expression level)", y = "ln(CAIS of gene)") + 
...
pcai <- ggplot(mousedf4, aes(x=log(expr), y=log(CAI))) + 
  labs( x = "log(expression level)", y = "ln(CAI of gene)") + 
  #annotate(geom="text", x=-12.5, y=0.7, parse=T, label="atop(R^2==0.57, p==0.0001)", color="black", size=6) +
  geom_point(alpha=0.05) + 
  geom_smooth() + 
  mytheme
