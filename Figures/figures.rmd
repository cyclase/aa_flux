...
dd <- cbind(d, metrics2[,2:146]) %>% mutate(fc_rodent = Glires-Root, fc_primate = Primatomorpha-Root, fc_rp = Glires-Primatomorpha, lt_Glires = logitTransform(Glires), lt_Primatomorpha = logitTransform(Primatomorpha), lt_Root = logitTransform(Root))

dd <- mutate(dd, logit_fc_rp = lt_Glires - lt_Primatomorpha)
...
model2 <- lm(logit_fc_rp ~ RaifordAerobicCost + VolumeA3 + Disorder_Propensity + mean_LKmarch22 + pGC, data = dm)
model3 <- lm(logit_fc_rp ~ RaifordAerobicCost + Disorder_Propensity + pGC, data = dm)
model4 <- lm(logit_fc_rp ~ RaifordAerobicCost + Disorder_Propensity + pGC + Essential_Costa2015, data = dm)
model5 <- lm(logit_fc_rp ~ RaifordAerobicCost + pGC, data = dm)
model6 <- lm(logit_fc_rp ~ RaifordAerobicCost + Disorder_Propensity, data = dm)
model7 <- lm(logit_fc_rp ~ Disorder_Propensity + pGC, data = dm)

modelc <- lm(logit_fc_rp ~ RaifordAerobicCost, data = dm)
modeld <- lm(logit_fc_rp ~ Disorder_Propensity, data = dm)
modelg <- lm(logit_fc_rp ~ pGC, data = dm)

modelv <- lm(logit_fc_rp ~ VolumeA3, data = dm)
models <- lm(logit_fc_rp ~ Levy_Stickiness, data = dm)
modelp <- lm(logit_fc_rp ~ Polarity, data = dm)
modelk <- lm(logit_fc_rp ~ mean_LKmarch22, data = dm)
...
cor.test(dm$RaifordAerobicCost,dm$logit_fc_rp, method='pearson') #R=-0.88, p=4e-07
cor.test(log10(dm$RaifordAerobicCost),dm$logit_fc_rp, method='pearson') #R=-0.87, p=7e-07
cor.test(dm$Disorder_Propensity,dm$logit_fc_rp, method='pearson') #R=0.75, p=0.0001
cor.test(dm$pGC,dm$logit_fc_rp, method='pearson') #R=0.62, p=0.003
cor.test(dm$VolumeA3,dm$logit_fc_rp, method='pearson') #R=-0.78, p=4e-05
cor.test(dm$weight,dm$logit_fc_rp, method='pearson') #R=-0.70, p=0.0005

t.test(x=subset(dm, Essential_Costa2015=="Essential")$logit_fc_rp, y=subset(dm, Essential_Costa2015=="Non-essential")$logit_fc_rp, var.equal = T) #p=0.02
cor.test(dm$Levy_Stickiness,dm$logit_fc_rp, method='pearson') #R=-0.60, p=0.005
cor.test(dm$Polarity,dm$logit_fc_rp, method='pearson') #R=-0.54, p=0.01
cor.test(dm$mean_LKmarch22,dm$logit_fc_rp, method='pearson') #R=0.62, p=0.003
```
...
###Figure 2
```{r Figure 2}
cor.test(dm$Disorder_Propensity,dm$logit_fc_rp, method='pearson') #R=0.75, p=0.0001 ->R2=0.57
cor.test(dm$Levy_Stickiness,dm$logit_fc_rp, method='pearson') #R=-0.60, p=0.005 ->R2=0.36
cor.test(dm$VolumeA3,dm$logit_fc_rp, method='pearson') #R=-0.78, p=4e-05 ->R2=0.61
cor.test(dm$Polarity,dm$logit_fc_rp, method='pearson') #R=-0.54 p=0.01 ->R2=0.29
cor.test(dm$pGC,dm$logit_fc_rp, method='pearson') #R=0.63, p=0.003 ->R2=0.40
cor.test(dm$mean_LKmarch22,dm$logit_fc_rp, method='pearson') #R=0.63, p=0.003 ->R2=0.39
cor.test(dm$resid_LKmarch22,dm$logit_fc_rp, method='pearson') #ns

cor.test(dm$RaifordAerobicCost,dm$logit_fc_rp, method='pearson') #R=-0.87, p=4e-07 ->R2=0.77
cor.test(log10(dm$RaifordAerobicCost),dm$logit_fc_rp, method='pearson') #R=-0.87, p=7e-07 ->R2=0.76
hist(dm$RaifordAerobicCost)
b <- boxcox(lm(dm$RaifordAerobicCost ~ 1))
...
dm$bcCost <- bc(dm$RaifordAerobicCost,lambda)
hist(dm$bcCost)

cor.test(dm$bcCost,dm$logit_fc_rp, method='pearson') #R=-0.85, p=2e-06

t.test(x=subset(dm, Essential_Costa2015=="Essential")$logit_fc_rp, y=subset(dm, Essential_Costa2015=="Non-essential")$logit_fc_rp, var.equal = T) #p=0.02

cor.test(dm$Mean_RSA,dm$logit_fc_rp, method='pearson') #R=-0.51, p=0.02
cor.test(dm$Hydrophobicity,dm$logit_fc_rp, method='pearson') #ns
cor.test(dm$TourasseChangeability,dm$logit_fc_rp, method='pearson') #R=0.58, p=0.008
cor.test(dm$num_Codons,dm$logit_fc_rp, method='pearson') #R=0.46, p=0.04
cor.test(dm$Mean_TdS_conf,dm$logit_fc_rp, method='pearson') #ns
cor.test(dm$Absolute_TS,dm$logit_fc_rp, method='pearson') #ns
cor.test(dm$N1redun,dm$logit_fc_rp, method='pearson') #ns
cor.test(dm$N2redun,dm$logit_fc_rp, method='pearson') #ns
cor.test(dm$N3redun,dm$logit_fc_rp, method='pearson') #R=0.64, p=0.002

cor.test(dm$logit_fc_rp,dm$pGC, method='pearson') #R=0.63, p=0.002
...
cor.test(dm$Jansen_Gerstein_enrichmentrank,dm$logit_fc_rp, method='pearson') #ns
cor.test(dm$Cherry_cc,dm$logit_fc_rp, method='pearson') #ns
...
Fig2a <- ggplot(dm, aes(x=RaifordAerobicCost, y=logit_fc_rp)) + 
  labs( x = "Cost in phosphate bonds", y = "Frequency difference (ML)") + 
  annotate(geom="text", x=50, y=0.2, parse=T, label="atop(R^2==0.76, p==7e-07)", color="black", size=6) +
  geom_point(size=3) + 
...
Fig2c <- ggplot(dm, aes(x=Disorder_Propensity, y=logit_fc_rp)) + 
  labs( x = "Disorder propensity", y = "Frequency difference (ML)") + 
  annotate(geom="text", x=0.15, y=0.2, parse=T, label="atop(R^2==0.57, p==0.0001)", color="black", size=6) +
  geom_point(size=3) + 
  geom_text(aes(label=aa),hjust=-0.1, vjust=0, color="blue", size = 6) + 
  mytheme
...
Fig2e <- ggplot(dm, aes(x=pGC, y=logit_fc_rp)) + 
  labs( x = "%GC", y = "Frequency differefnce (ML)") + 
  annotate(geom="text", x=0.25, y=0.2, parse=T, label="atop(R^2==0.40, p==0.003)", color="black", size=6) +
  geom_point(size=3) + 
  geom_text(aes(label=aa),hjust=-0.1, vjust=0, color="blue", size = 6) + 
  mytheme
...
cor.test((dm$lt_Primatomorpha - dm$lt_Root), (dm$lt_Glires - dm$lt_Root), method='spearman') #R=0.43 p=0.06
...
Fig3c <- ggplot(dm, aes(x=Pnoratio, y=Rnoratio))+
  ...
  coord_equal() +
  geom_abline(data=subset(reg, Method=="MA"), aes(intercept=Intercept, slope=Slope), color="red", linewidth=1.5) + 
...
Fig3d <- ggplot(dm, aes(x=lt_Primatomorpha-lt_Root, y=lt_Glires-lt_Root))+
  ...
  coord_equal() +
  geom_abline(data=subset(mlreg, Method=="MA"), aes(intercept=Intercept, slope=Slope), color="red", linewidth=1.5) + 
  ...
  annotate(geom="text", x=0.3, y=-0.2, parse=T, label="atop(R^2==0.19, p==0.05)", color="black", size=6) + 
  #geom_pointrange(aes(xmin=Pnoratio-Perror, xmax=Pnoratio+Perror)) + 
  #geom_pointrange(aes(ymin=Rnoratio-Rerror, ymax=Rnoratio+Rerror)) + 
  geom_point(size=3) + 
  geom_text(aes(label=aa),hjust=-0.1, vjust=0, color="blue", size = 6) +
...
dm$lt_Primdiff <- dm$lt_Primatomorpha - dm$lt_Root
dm$lt_Glirdiff <- dm$lt_Glires - dm$lt_Root

mlfitII <- lmodel2(lt_Glirdiff ~ lt_Primdiff, data=dm, range.y="interval", range.x="interval", nperm=99)
mlfit <- lm(lt_Glirdiff ~ lt_Primdiff, data=dm)

mlreg <- mlfitII$regression.results
mlslope <- subset(mlreg, Method=="MA")$Slope
mlintercept <- subset(mlreg, Method=="MA")$Intercept

dm$mlresiduals_m2<-residuals_from_model_2(dm$lt_Primdiff, dm$lt_Glirdiff, mlslope, mlintercept)
dm$mlprojections_m2<- projections_from_model_2(dm$lt_Primdiff, dm$lt_Glirdiff, mlslope, mlintercept)

#account for negative signs
dm[dm$aa=="D", "mlprojections_m2"] <- -0.10727265
dm[dm$aa=="E", "mlprojections_m2"] <- -0.19518533
dm[dm$aa=="F", "mlprojections_m2"] <- -0.17054453
dm[dm$aa=="I", "mlprojections_m2"] <- -0.35973935
dm[dm$aa=="K", "mlprojections_m2"] <- -0.18797254
dm[dm$aa=="L", "mlprojections_m2"] <- -0.03273869
dm[dm$aa=="M", "mlprojections_m2"] <- -0.26276027
dm[dm$aa=="N", "mlprojections_m2"] <- -0.11482695
dm[dm$aa=="Y", "mlprojections_m2"] <- -0.36517081

cor.test(dm$projections_m2, dm$luca_usage, method='pearson') #ns
cor.test(dm$projections_m2, dm$preluca_usage, method='pearson') #ns
cor.test(dm$projections_m2, dm$TrifonovConsensusOrderOfRecruitment, method='pearson') #R=0.49, p=0.03
cor.test(dm$mlprojections_m2, dm$luca_usage, method='pearson') #ns
cor.test(dm$mlprojections_m2, dm$preluca_usage, method='pearson') #ns
cor.test(dm$mlprojections_m2, dm$TrifonovConsensusOrderOfRecruitment, method='pearson') #ns

cor.test(dm$mlprojections_m2, dm$projections_m2, method='pearson') #ns
summary(lm(mlprojections_m2 ~ projections_m2, data=dm))


summary(lm(preluca_usage ~ projections_m2, data=dm, weights = preluca_usage_se))
summary(lm(preluca_usage ~ Jordan_st1_D, data=dm, weights = preluca_usage_se))
summary(lm(luca_usage ~ projections_m2, data=dm, weights = luca_usage_se))
summary(lm(luca_usage ~ Jordan_st1_D, data=dm, weights = luca_usage_se))
...
Fig5a<-ggplot(dm, aes(x=residuals_m2, y=kldCAISslope_Proteome)) +
  ...
  geom_segment(aes(x=subset(dm, aa=="D")$residuals_m2, 
                 y=subset(dm, aa=="D")$kldCAISslope_Proteome, 
                 xend=subset(dm, aa=="E")$residuals_m2, 
                 yend=subset(dm, aa=="E")$kldCAISslope_Proteome), color="grey80", alpha=0.3, size=1.5) + 
  ...
  annotate(geom="text", x=0.1, y=ypos_lo(dm$kldCAISslope_Proteome), parse=T, label="atop(R^2==0.50, p==5e-04)", color="black", size=6) +  geom_pointrange(aes(xmin=residuals_m2-residual_error, xmax=residuals_m2+residual_error)) +
...
Fig5b<-ggplot(dm, aes(x=logit_fc_rp, y=kldCAISslope_Proteome)) +
  ...
  geom_segment(aes(x=subset(dm, aa=="K")$logit_fc_rp, 
                 ...
                 yend=subset(dm, aa=="R")$kldCAISslope_Proteome), color="grey80", alpha=0.3, size=1.5) + 
  geom_segment(aes(x=subset(dm, aa=="D")$logit_fc_rp, 
                 y=subset(dm, aa=="D")$kldCAISslope_Proteome, 
                 xend=subset(dm, aa=="E")$logit_fc_rp, 
                 yend=subset(dm, aa=="E")$kldCAISslope_Proteome), color="grey80", alpha=0.3, size=1.5) + 
  geom_segment(aes(x=subset(dm, aa=="I")$logit_fc_rp, 
                 y=subset(dm, aa=="I")$kldCAISslope_Proteome, 
                 xend=subset(dm, aa=="V")$logit_fc_rp, 
                 yend=subset(dm, aa=="V")$kldCAISslope_Proteome), color="grey80", alpha=0.3, size=1.5) + 
  annotate(geom="text", x=0.15, y=-0.2, parse=T, label="atop(R^2==0.27, p==0.02)", color="black", size=6) +
  geom_pointrange(aes(ymin=kldCAISslope_Proteome-kldCAISslope_Proteome_Std.Error, ymax=kldCAISslope_Proteome+kldCAISslope_Proteome_Std.Error)) +
  geom_point(size=3) + 
  geom_text(aes(label=aa),hjust=-0.1, vjust=0, color="blue", size = 6) + 
  mytheme

Fig5c<-ggplot(dm, aes(x=logit_fc_rp, y=residuals_m2)) +
  ...
  geom_segment(aes(x=subset(dm, aa=="D")$logit_fc_rp, 
                 y=subset(dm, aa=="D")$residuals_m2, 
                 xend=subset(dm, aa=="E")$logit_fc_rp, 
                 yend=subset(dm, aa=="E")$residuals_m2), color="grey80", alpha=0.3, size=1.5) + 
  geom_segment(aes(x=subset(dm, aa=="I")$logit_fc_rp, 
                 y=subset(dm, aa=="I")$residuals_m2, 
                 xend=subset(dm, aa=="V")$logit_fc_rp, 
                 yend=subset(dm, aa=="V")$residuals_m2), color="grey80", alpha=0.3, size=1.5) + 
  annotate(geom="text", x=0.15, y=-0.1, parse=T, label="atop(R^2==0.26, p==0.02)", color="black", size=6) +
