...
pcais <- ggplot(mousedf4, aes(x=log10(expr), y=log10(CAIS))) + 
  ...
  annotate(geom="text", x=3, y=-1.25, parse=T, label="atop(R^2==0.10, p<2e-16)", color="black", size=6) +
  geom_point(alpha=0.05, stroke=0, size=2) + 
  geom_smooth(color="gray40") +
  geom_segment(aes(x=1.5, 
                 y=(0.141237*1.5)-1.018667, 
                 xend=log10(max(expr)),
                 yend=(0.141237*log10(max(expr)))-1.018667), linewidth=0.5) + 
...
pcai <- ggplot(mousedf4, aes(x=log10(expr), y=log10(CAI))) + 
  ...
  annotate(geom="text", x=3, y=-0.25, parse=T, label="atop(R^2==0.07, p<2e-16)", color="black", size=6) +
  geom_point(alpha=0.05, stroke=0, size=2) + 
  geom_smooth(color="gray40") + 
  geom_segment(aes(x=1.5, 
                 y=(0.026273*1.5)-0.178934, 
                 xend=log10(max(expr)),
                 yend=(0.026273*log10(max(expr)))-0.178934), linewidth=0.5) + 
  scale_x_continuous(labels = label_math(10^.x), breaks = c(-6,-4,-2,0,2,4,6)) +
  scale_y_continuous(labels = label_math(10^.x)) +
  mytheme
...
myggsave_gr <- function(x){
  ggsave(plot=x, filename=paste0(deparse(substitute(x)), ".svg"), path="C:/Users/hanon/Documents/Github/aa_flux/supp_analyses/cais-expr", device="svg", height=4, width=6, units="in")
}
