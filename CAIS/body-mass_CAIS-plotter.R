...
dfno_weights<-read.csv(file="CAIS_KLD.txt", header=T)
str(dfno_weights)
...
colnames(dfno_weights)[1] <- 'SpeciesUID'
dfmass<-merge(dfmass, dfno_weights, by.x="SpeciesUID", by.y="SpeciesUID")
str(dfmass)

dfmass <- subset(dfmass, NewickName != "Pantholops_hodgsonii") #remove antelope
...
dfmass2 <- arrange(dfmass, match(NewickName, phyloorder))
dfmass2

#Set up data into vectors for use in PIC
massvector<-as.vector(log10(dfmass$X5.1_AdultBodyMass_g))
massvector2<-as.vector(log10(dfmass2$X5.1_AdultBodyMass_g))

CAISvector<-as.vector(dfmass$complete_CAIS)
CAISvector2<-as.vector(dfmass2$complete_CAIS)
...
piccommandmass2<-pic(massvector2,trimmedtree)
piccommandCAIS2<- pic(CAISvector2,trimmedtree)
...
lmiPIC2<-lm(piccommandmass2~ piccommandCAIS2-1)
summary(lmiPIC2)
cor.test(piccommandmass2, piccommandCAIS2, method = 'pearson')
cor.test(piccommandmass2, piccommandCAIS2, method = 'spearman')
...
massplot3 <-ggplot(data=lmiPIC2, aes(piccommandCAIS2,piccommandmass2)) + geom_point(size=2) + labs( x = "CAIS [PIC Corrected]", y = "Adult Body Mass\n[PIC Corrected]") + stat_smooth(method = "lm", color="red")
...
massplot3<- massplot3 + geom_text(data=lmiPIC, aes(x = -0.0005, y = -0.8, label = "Pearson's R:-0.33\n p-value:0.0065"),size=7, colour = 'black')+ theme(axis.text = element_text(size = 24)) 
massplot3
